...
public class WriteReplyToCommentActivity extends
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (bean == null) {
			replyDraftBean = (ReplyDraftBean) getIntent().getSerializableExtra(
					"draft");
		...
		getEditTextView().setHint(
				"@" + bean.getUser().getScreen_name() + "£º" + bean.getText());
	...
	protected CommentBean sendData() throws WeiboException {
		ReplyToCommentMsgDao dao = new ReplyToCommentMsgDao(token, bean,
				((EditText) findViewById(R.id.status_new_content)).getText()
						.toString());
	...
	private ItemBean repost() throws WeiboException {
		...
		RepostNewMsgDao dao = new RepostNewMsgDao(token, bean.getStatus()
				.getId());
